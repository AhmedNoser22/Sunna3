...
@Component({
  ...
  template: `
    <button (click)="signInWithGoogle()" [disabled]="loading">
      ...
      <span>{{ loading ? 'جارٍ التسجيل...' : 'المتابعة باستخدام جوجل' }}</span>

      <!-- زر Google الحقيقي مخفي — بس موجود عشان يشتغل -->
...
export class GoogleBtn implements OnInit {
  ...
  ngOnInit() {
    ...
    // render الزر الحقيقي في div مخفي — ده اللي بيشغل الـ flow صح
  ...
  signInWithGoogle() {
    this.loading = true;
    // اضغط الزر الحقيقي المخفي برمجياً
    const hiddenBtn = document.querySelector('#google-btn-hidden div[role="button"]') as HTMLElement;
    if (hiddenBtn) {
      hiddenBtn.click();
      this.loading = false;
    } else {
