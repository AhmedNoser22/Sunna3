...
type ActiveTab = 'tickets' | 'vendors';
...
export class Dashboard implements OnInit {
  ...
  filteredTickets = computed(() => {
    ...
    if (q)            list = list.filter(t =>
      ...
      t.tenantLocation?.toLowerCase().includes(q)
    );
    return list;
  });
  ...
  loadTickets() {
    ...
    this.http.get<Ticket[]>(`${this.API_URL}/api/Tickets`, { headers: this.headers() })
      .subscribe({
  ...
  loadVendors() {
    ...
    this.http.get<Vendor[]>(`${this.API_URL}/api/Vendors`, { headers: this.headers() })
      .subscribe({
